...
public class LingerBrain implements IHerdBrain
{
	public Herd herd;
	...
	public Center homeLocation;

	int hoursWaitingOnHerd = 0;

	public LingerBrain(Herd h, Center curLoc)
	{
		herd = h;
		activityTimer = 1;
	...
	public void process(World world, IslandMap map, long currentHour)
	{
		...
		if(activityTimer <= 0)
		{
			if(currentActivity == HerdActivityEnum.WORKING)//The herd is ready to leave the current need zone
			{
				...
				//========Create a new goal========
				//1. Find a hex near the home hex to wander to
				Vector<Center> goalZones = map.filterRange(map.landCenters(map.centers), homeLocation, 300);
				...
				if(path == null)
					return;
				currentGoal = new HerdGoal(HerdGoalEnum.REST, goalLoc, new HerdPath(currentHour, path));
