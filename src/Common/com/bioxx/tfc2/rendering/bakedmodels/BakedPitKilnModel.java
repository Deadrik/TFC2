...
public class BakedPitKilnModel implements IPerspectiveAwareModel 
{
	...
	protected IBakedModel getActualModel(IBlockState state, List<PropertyItem.PItem> items) {
		...
		if(items != null && !items.isEmpty()) {
			...
			{
				builder.add(new TRSRBakedModel(item.model, item.x, item.y-0.48f, item.z, item.r, (float) (Math.PI), 0, item.s), null, 0);
			}
	...
	private static class TableItemOverrideList extends ItemOverrideList {
		...
		public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, World world, EntityLivingBase entity) {
			if(originalModel instanceof BakedPitKilnModel) {
				return ((BakedPitKilnModel) originalModel).getActualModel(null, Collections.<PropertyItem.PItem>emptyList());
			}
